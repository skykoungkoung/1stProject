<%@page import="gongmo.community.model.MakeTeamDTO"%>
					...
					<div class="commuBody">
						<table class="allCommuList">
							...
							<%if(count>0){for(int i=0; i<articleList.size(); i++){
								...
								String gmName = dao.getGMName(article.getTargetGM());
								String[] pnow = null;
								if(article.getPnow()!=null){
									pnow = article.getPnow().split(",");
								}
								...
								<tr>
									...
									<td><%= gmName %></td>
									<td>
										<a href="../gongmo/gongmoTeamContent.jsp?gmboard_num=<%= article.getTargetGM() %>&maketeam_num=<%= article.getNum() %>">
										<%= article.getTitle() %></a>
									</td>
									<td>
										<a href="../msg/sendmsgForm.jsp?send=<%=sender%>&receive=<%= article.getId() %>"
										onclick="window.open(this.href, '메시지 보내기', 'width=500px,height=500px,toolbars=no,scrollbars=no'); return false;"
										><%= article.getId() %></a>
									</td>
									<td><%= article.getPtotal() %>/
									<%if(pnow==null){ %>
										0</td>
									<%}else{ %>
										<%= pnow.length %></td>
